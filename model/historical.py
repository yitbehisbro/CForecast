from .forecast7days import coordinate, weather_code
import requests
import spacy
import dateparser
import pandas as pd
...
def get_historical_data(city=None, start_date=None, end_date=None):
    ...
    url = "https://archive-api.open-meteo.com/v1/archive?latitude={}&longitude={}&start_date={}&end_date={}&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,rain_sum&timezone=Africa%2FCairo".format(coord["lat"], coord["lon"], start_date, end_date)
    ...
    for i in range(len(time)):
        text += "\n⌚ On {} (Week {}):\n\n\t🌞\t  Day name: {}\n\t⛅ \t  Weather: {}\n\t🌧 \t  Rain: {} mm\n\t🌡 \t  Max temp: {} ℃\n\t🌡 \t  Min temp: {} ℃\n\t🌘\t  Sun rised: {} UTC\n\t🌒\t  Sun setted: {} UTC\n\n".format(time[i], pd.Timestamp(time[i]).week, pd.Timestamp(time[i]).day_name(), weather_code(weathercode[i]), rain_sum[i], temp_max[i], temp_min[i], get_time(sunrise[i]), get_time(sunset[i]))
    return text
...
def return_historical_data(statement):
    ...
    for ent in statement.ents:
        ...
        if ent.label_ == 'GPE':  # Geopolotical entity
            city = ent.text
    print(dates)
    print(statement)
    ...
    if metadata["city"] and metadata["start_date"] and metadata["end_date"] is None:
        return "Minimum requirement not satsfied"
